...
import { getSortingFromTableSorting } from "./data-approval-list/DataSubscriptionList";
import { DataSubscriptionViewModel } from "./DataSubscriptionViewModel";
...
export const DataDifferencesList: React.FC<DataDifferencesListProps> = ({
    ...
    const baseConfig: TableConfig<DataDiffViewModel> = useMemo(
            ...
            actions: [
                {
                    ...
                    onClick: async (selectedIds: string[]) => {
                        ...
                        const result = await compositionRoot.malDataSubscription.duplicateValue(items, revoke);
        ...
        [compositionRoot.malDataSubscription, isMalAdmin, isUpdated, revoke, snackbar]
    );
    ...
    const getRows = useMemo(
        () => async (_search: string, paging: TablePagination, sorting: TableSorting<DataSubscriptionViewModel>) => {
            ...
            const { pager, objects } = await compositionRoot.malDataSubscription.getDiff({
                config,
        ...
        [compositionRoot.malDataSubscription, config, selectedIds, snackbar]
    );
    ...
    const saveReorderedColumns = useCallback(
        async (columnKeys: Array<keyof DataDiffViewModel>) => {
            await compositionRoot.malDataSubscription.saveColumns(Namespaces.MAL_DIFF_STATUS_USER_COLUMNS, columnKeys);
        },
        [compositionRoot]
    );
    ...
    useEffect(() => {
        compositionRoot.malDataSubscription.getColumns(Namespaces.MAL_DIFF_STATUS_USER_COLUMNS).then(columns => {
