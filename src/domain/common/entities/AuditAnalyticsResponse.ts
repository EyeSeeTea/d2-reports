...
export function getEventQueryString(
    ...
    const dimensionParams = [`pe:${period}`, `ou:${orgUnitId}`];
    const searchParams: any = { dimension: dimensionParams, stage: programStageId, pageSize: "100000" };
    const params = new URLSearchParams(searchParams).toString();

    const eventQueryString = `/analytics/events/query/${programId}.json?dimension=pe:${period}&dimension=ou:${orgUnitId}&stage=${programStageId}${query}&pageSize=100000`;
